...
#define Ns 147 // 147-10+1, min size 10
#define L2 L1*Ns
#define L3 545*Ns
#define n1 3	
...
int main() {
        FILE *f2,*f10,*f11,*f12;
        long int i,i1,i2,i3,j,k,dx,ks[2],l=147,L=L1,dw0,sze,sze1,soft[1000][3],nuc,ksm,ks1,t,w[n1],b1[n1],b2[n1],b3,hc1,hc2,cen[n1],h[n1],Nsim,b1in,b1out,b2in,b2out,a,amin,pk;
        double p[L1],p1,p0,Es[L1],Va,CN,mu,E0,Zf[L1],Zb[L1],Pn[L1],O[L1],Zfsum,Zbsum,Esum,cy,cy1,hmx1,hyx,hmn1,hmx,hmn,hy,sd,px[Ns],Px[Ns],hy1[2],GN0,sumG,dg;
        ...
        static double Eij[L1][Ns],subheat[L1][Ns],den5[L1][Ns],den3[L1][Ns],dya[L1][Ns],Subheat[2000][Ns],Den5[2000][Ns],Den3[2000][Ns],Dya[2000][Ns],Eseq[L1][Ns],niebE[L1][2],NiebE[2000][2],gn[2000][Ns],Gn[2000],GN[2000],Gp[5][2];
        ...
        E0=5.4/(147-91); mu=-11.6; CN=exp(mu); Nsim=1;  //E0=10.0/(147-91);  
        for(i1=0;i1<Nsim;i1++) {                                                      
           for(i=0;i<L1;i++) {
              for(t=0;t<nuc;t++) { 
                 Eij[i][t]=0; 
           ...
           for(t=0;t<nuc;t++) {  // subnucleosome
            ...
            b3=5000+100; pk=0; i1=0; i2=-1; GN0=0; sumG=0;
            for(i=0;i<2000;i++) {
            ...
            dg=0; j=0;
            for(i=0;i<4;i++) {
               printf("Gp %lf %lf %lf \n",Gp[i][0],Gp[i][1],sumG/2000);
               if(Gp[i+1][0]>0 && Gp[i][0]>0) {
                 dg=dg+Gp[i+1][0]-Gp[i][0]; j=j+1;
               }
            }
            if(j>0) {
              dg=dg/(double)j;
            }
            printf("Gp %lf %lf %lf %lf \n",Gp[4][0],Gp[4][1],sumG/2000,dg);
