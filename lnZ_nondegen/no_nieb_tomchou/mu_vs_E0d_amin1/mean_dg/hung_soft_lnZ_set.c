...
int main() {
        ...
        for(h=hx;h<(hx+2);h=h+2) {  
           E0=(double)h/(3.836*(147-1));
           for(w=0;w<500;w=w+2) {
              ...
              pO=0; pO1=0; //printf("t %ld dx %ld gsum %lf w %ld \n",t,dx,gsum1,w);
              ...
              //printf("ZfL %lf nuc %ld ksm %ld lx %lf pO %lf lm %lf dg %lf h %ld w %ld \n",Zf[L-1],nuc,ksm,lx,pO,lm,dg,h,w); 
           } 
           printf("ZfL %lf nuc %ld ksm %ld pO %lf dg %lf pk %ld h %ld w %ld \n",Zf[L-1],nuc,ksm,pO,dg,pk,h,w);    
        }
        fclose(f4); return(0); 
}
