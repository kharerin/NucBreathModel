...
mu1=[]
E01=[]
f = open('/media/hungyo/edrive/hung_ens_laptop/hungyo_nucdyn/NucleoModel/lnZ_nondegen/mu_vs_E0d/occ_0p8.txt','r')
for row in f:
    row = row.split(' ')
...
for i in range(250*250):
    ...
    if (lm[i]>124 and lm[i]<126) and (O[i]>0.75 and O[i]<0.85) and (pO1[i]>0.006 and pO1[i]<0.0065): # and pO1[i]>0.006 and pO1[i]<0.0065 and O[i]>0.799 and O[i]<0.801:
       #mu1xr.append(mu2[i])
       #E01xr.append(E02[i])
       print('>>>>>>>>>1st model...i...mu and e0...occ..',mu2[i],E02[i],O[i])
       j1=j1+1   
       
print('j...j1...j2...j3',j,j1,j2,j3)
n=-56
...
plt.ylabel('$\epsilon_{0}$',fontsize=12)
...
plt.plot(mu1xx,E11xx, linewidth=1.5, c = 'r',zorder=0, label='$\mu$*') #linestyle='dashed'
...
mu1=[]
E01=[]
f = open('/media/hungyo/edrive/hung_ens_laptop/hungyo_nucdyn/NucleoModel/lnZ_nondegen/no_nieb_tomchou/mu_vs_E0d/occ_0p8.txt','r')
for row in f:
    row = row.split(' ')
    mu1.append(float(row[0]))
    E01.append(float(row[1]))
...
for i in range(250*250):
    ...
    if (lm[i]>124 and lm[i]<126) and (O[i]>0.75 and O[i]<0.85) and (pO1[i]>0.006 and pO1[i]<0.0065): # and pO1[i]>0.006 and pO1[i]<0.0065 and O[i]>0.799 and O[i]<0.801:
       #mu1xr.append(mu2[i])
       #E01xr.append(E02[i])
       print('>>>>>>>>>2nd model...i...mu and e0...occ..',mu2[i],E02[i],O[i])
       j1=j1+1 
print('j...j1...j2...j3',j,j1,j2,j3)
n=-56
E11x=np.multiply(E01x,n) 
...
mu1=[]
E01=[]
f = open('/media/hungyo/edrive/hung_ens_laptop/hungyo_nucdyn/NucleoModel/lnZ_nondegen/no_nieb_tomchou/mu_vs_E0d_amin1/occ_0p8.txt','r')
for row in f:
    row = row.split(' ')
    mu1.append(float(row[0]))
    E01.append(float(row[1]))
...
#plt.plot(mu1,E11, linewidth=1, c = 'k',label='80%')
#plt.plot(mu1xO2,E11xO2, linewidth=1, linestyle='dashed', c = 'k',label='75%')
model3 = np.poly1d(np.polyfit(mu1xO3, E11xO3, 11))
...
#plt.xticks([])
#plt.yticks([])
plt.plot(mu1xx,E11xx, linewidth=1.5, c = 'r', label='$\mu$*') #linestyle='dashed'
plt.legend(loc="lower left",fontsize=9,frameon=True) #bbox_to_anchor=(1.4, -0.1)
